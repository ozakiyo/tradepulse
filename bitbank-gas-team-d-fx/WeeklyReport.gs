/**
 * チームD-FX: 週次レポート
 * D4F_売買履歴から 7日間 / 前日 / 今月 を集計。
 */

var D4F_SHEET_REPORT = 'D4F_週次レポート';
var D4F_SHEET_DAILY = 'D4F_日次レポート';
var D4F_SHEET_MONTHLY = 'D4F_月次レポート';
var D4F_REPORT_MAX_TRADES = 100;
var D4F_TRADE_COL = { time: 0, side: 1, price: 2, amount: 3, memo: 5 };

function d4fCollectReportTrades_() {
  var range = rptGetPeriodRange_('7d');
  return d4fCollectReportTradesForPeriod_(range.from, range.to);
}

function d4fCollectReportTradesForPeriod_(from, to) {
  return rptCollectTradesFromSheet_(D4F_SHEET_TRADE, D4F_TRADE_COL, from, to, 'single');
}

function d4fAnalyzeTrades_(trades) {
  var results = [];
  for (var i = 0; i < Math.min(trades.length, D4F_REPORT_MAX_TRADES); i++) {
    var t = trades[i];
...
function d4fGenerateRecommendations_(analyses) {
  var recs = [];
...
function d4fWritePeriodSheet_(sheetName, title, trades, stats) {
  ...
  rptWriteTradeDetailBlock_(sheet, row, trades, D4F_REPORT_MAX_TRADES);
}

function d4fWriteReport_(trades, analyses, recs, periodBundle) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName(D4F_SHEET_REPORT);
  if (!sheet) sheet = ss.insertSheet(D4F_SHEET_REPORT);
  sheet.clear();
  ...
  d4fLog_('週次レポート出力完了 (' + trades.length + '件)');
}

function d4fRunReport_() {
  var range7 = rptGetPeriodRange_('7d');
  var rangeY = rptGetPeriodRange_('yesterday');
  var rangeM = rptGetPeriodRange_('month');

  var trades7d = d4fCollectReportTradesForPeriod_(range7.from, range7.to);
  var tradesY = d4fCollectReportTradesForPeriod_(rangeY.from, rangeY.to);
  var tradesM = d4fCollectReportTradesForPeriod_(rangeM.from, rangeM.to);
  ...
  if (trades7d.length === 0 && tradesY.length === 0 && tradesM.length === 0) {
    ...
    d4fReportToMeta_('7日間', 0, '-', '-', 0, 0, '取引なし');
    d4fReportToMeta_('前日', 0, '-', '-', 0, 0, '取引なし');
    d4fReportToMeta_(rangeM.label, 0, '-', '-', 0, 0, '取引なし');
    return;
  }

  var analyses = d4fAnalyzeTrades_(trades7d);
  var recs = d4fGenerateRecommendations_(analyses);
  d4fWriteReport_(trades7d, analyses, recs, periodBundle);
  d4fWritePeriodSheet_(D4F_SHEET_DAILY, '【D4F 前日レポート】', tradesY, statsY);
  d4fWritePeriodSheet_(D4F_SHEET_MONTHLY, '【D4F 月次レポート】', tradesM, statsM);

  var recText = recs.length > 0 ? recs[0].reason : '-';
  d4fReportToMeta_('7日間', stats7d.closedCount, stats7d.closedCount > 0 ? stats7d.winRate.toFixed(1) : '-', stats7d.pf, stats7d.netPnl, stats7d.avgHoldH, recText);
  d4fReportToMeta_('前日', statsY.closedCount, statsY.closedCount > 0 ? statsY.winRate.toFixed(1) : '-', statsY.pf, statsY.netPnl, statsY.avgHoldH, statsY.closedCount > 0 ? '前日決済 ' + statsY.closedCount + '件' : '前日決済なし');
  d4fReportToMeta_(rangeM.label, statsM.closedCount, statsM.closedCount > 0 ? statsM.winRate.toFixed(1) : '-', statsM.pf, statsM.netPnl, statsM.avgHoldH, rangeM.label + ' 累計');
}

function d4fReportToMeta_(period, tradeCount, winRate, pf, netPnl, avgHoldH, recommendation) {
  ...
  try {
    ...
    d4fLog_('META統合レポートに送信(' + period + ')');
  } catch (e) {
    d4fLog_('META統合レポート送信失敗: ' + e.message);
  }
}
