...
function d4fDetectHookLaws_(segs, cfg) {
  var events = [];
  ...
  for (var i = 0; i < segs.length; i++) {
    var seg = segs[i];
    var band = d4fGetKagiBand_(seg.to, cfg.kagiBaseStep);
...
function d4fCountRecentLaws_(events, side, fromIdx) {
  var count = 0;
  var codes = [];
...
function d4fHadPullback_(segs, lookback) {
...
function d4fAnalyzeShibata_(candles1h, cfg, lastPrice) {
  ...
  var segs = d4fBuildShibataKagi_(closed, cfg.kagiBaseStep);
  var events = d4fDetectHookLaws_(segs, cfg);
  var fromIdx = Math.max(0, segs.length - cfg.lawLookback);
  var buyRecent = d4fCountRecentLaws_(events, 'buy', fromIdx);
  var sellRecent = d4fCountRecentLaws_(events, 'sell', fromIdx);

  var n = closed.length - 1;
  var price = lastPrice != null ? lastPrice : closed[n].close;
  var lowerLine = d4fBuildLowerTrendline_(candles1h, D4F_CONFIG.TRENDLINE_LOOKBACK);
  var upperLine = d4fBuildUpperTrendline_(candles1h, D4F_CONFIG.TRENDLINE_LOOKBACK);
  var lineVal = lowerLine ? lowerLine.valueAt(n) : null;
  var aboveSupport = lineVal == null || price > lineVal;

  var ikiBuy = false;
  if (upperLine && d4fIsBullishBody_(closed[n])) {
    var upperVal = upperLine.valueAt(n);
    var bodyTop = Math.max(closed[n].open, closed[n].close);
    if (bodyTop > upperVal) ikiBuy = true;
  }

  var secondStage = d4fHadPullback_(segs, cfg.lawLookback);
  ...
  if (lowerLine && d4fIsBearishBody_(closed[n])) {
  ...
  if (lineVal != null) noteParts.push('下値斜線' + d4fFormatPrice_(lineVal));
  ...
  return {
    ...
    trendline: lineVal != null ? d4fFormatPrice_(lineVal) : null,
