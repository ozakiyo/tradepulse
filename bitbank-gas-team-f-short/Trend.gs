/**
 * チームF-FX-Short: エントリー・決済（マルチ銘柄対応）
 * ロング/ショート両対応。
 * エントリー: 1Hトレンド方向 + 5m戻り確定
 * 決済: 30mダウ理論でトレンド崩壊/反転を検出（Main.gsで判定）
 */

function f6RunTrend_(candles1h, ticker, assets, cfg, state, analysis, activeSymbols) {
  ...
  var symbols = activeSymbols || f6GetActiveSymbols_();
  var openCount = f6CountOpenPositions_(symbols);
  if (openCount >= cfg.maxOpenPositions) {
    f6Log_('同時保有上限(' + cfg.maxOpenPositions + ') 現在' + openCount + ' — 新規見送り');
    state.lastSignal = 'pause';
    return { signal: 'none', note: '同時保有上限(' + openCount + '/' + cfg.maxOpenPositions + ')' };
  }
