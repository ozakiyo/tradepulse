...
/** ロング損切: Bid が 1H 下限 − buffer% を下回る */
export function isH1AdverseBreakBuffered_(close, h1, side, spreadOpts, bufferPct = 0) {
  if (!h1?.high || !h1?.low) return false;
  const buf = Math.max(0, Number(bufferPct) || 0) / 100;
  const spread = spreadAmount_(spreadOpts);
  if (side === 'long') {
    const bid = spread > 0 ? close - spread : close;
    return bid < h1.low * (1 - buf);
  }
  if (side === 'short') {
    const ask = spread > 0 ? close + spread : close;
    return ask > h1.high * (1 + buf);
  }
  return false;
}

/** 直近 confirmBars 本の確定1H足がすべてバッファ付き1Hブレイク */
export function isH1AdverseBreakConfirmed_(
  closed1hBars,
  h1,
  side,
  spreadOpts,
  ...
  const n = Math.max(1, Math.floor(confirmBars) || 1);
  if (!closed1hBars?.length || closed1hBars.length < n) return false;
  const slice = closed1hBars.slice(-n);
  return slice.every((bar) =>
    isH1AdverseBreakBuffered_(bar.close, h1, side, spreadOpts, bufferPct)
  );
}

/** @deprecated isH1AdverseBreakBuffered_(..., 0) を使用 */
export function isH1AdverseBreak_(close, h1, side, spreadOpts) {
  return isH1AdverseBreakBuffered_(close, h1, side, spreadOpts, 0);
}
