...
/** 確定済み1H足を新しい順で count 本（1H損切確認用） */
export function getLastClosed1hList_(candles1h, count) {
  const n = Math.max(1, Math.floor(count));
  if (!candles1h || candles1h.length < n + 1) return [];
  const end = candles1h.length - 2;
  const start = end - n + 1;
  if (start < 0) return [];
  return candles1h.slice(start, end + 1).map((c) => ({
    close: c.close,
    time: c.time,
    high: c.high,
    low: c.low,
