 ...
 * @param {string} pairMode 'grid' | 'single' | 'symbol' | 'symbolFifo'
 */
function rptCollectTradesFromSheet_(sheetName, colMap, from, to, pairMode) {
  ...
  if (pairMode === 'symbolFifo') return rptCollectSymbolFifoTradesForPeriod_(sheetName, colMap, from, to);
...
function rptCollectSymbolFifoTradesForPeriod_(sheetName, colMap, from, to) {
  var allRows = rptSortTradeRowsByTime_(rptReadAllTradeRowsFromSheet_(sheetName, colMap));
  var paired = rptPairSymbolFifoTrades_(allRows);
  var result = [];
...
function rptPairSymbolFifoTrades_(rows) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    var remainToSell = r.amount > 0 ? r.amount : openBySym[sym][0].amount;
    while (remainToSell > 0.00000001 && openBySym[sym].length) {
      var entry = openBySym[sym][0];
      var sold = Math.min(remainToSell, entry.amount);
      var pnlJpy = (r.price - entry.price) * sold;
      trades.push({
        symbol: sym,
        entryTime: entry.time,
        exitTime: r.time,
      ...
      entry.amount -= sold;
      remainToSell -= sold;
      if (entry.amount < 0.00000001) openBySym[sym].shift();
    }
  }
  ...
  Object.keys(openBySym).forEach(function (sym) {
    ...
    for (var j = 0; j < queue.length; j++) {
      var entry = queue[j];
      if (entry.amount < 0.00000001) continue;
      trades.push({
        symbol: sym,
        entryTime: entry.time,
        exitTime: null,
...
function rptRoundTripKey_(trade) {
  return [trade.symbol, trade.side, trade.entryTime, trade.entryPrice].join('\0');
}

/** 半分損切を含む1エントリーを1取引として PF・勝率を集計 */
function rptCalcRoundTripStats_(trades) {
  var groups = {};
  var groupHoldH = {};
  var openCount = 0;
