function e5CalcEma_(values, period) {
...
function e5CalcRsi_(closes, period) {
...
function e5CalcEr_(closes, period) {
  if (closes.length < period + 1) return null;
...
function e5CalcAdx_(candles, period) {
...
function e5BarsPer4h_(candleType) {
  return candleType === '15min' ? 16 : 4;
}

function e5Aggregate4h_(candles, barsPer4h) {
  var n = barsPer4h || 4;
  var out = [];
...
function e5Calc4hBias_(candles, candleType) {
  var bars4h = e5Aggregate4h_(candles, e5BarsPer4h_(candleType));
  if (bars4h.length < 55) return 'neutral';
  var closes = bars4h.map(function (c) {
    return c.close;
  });
  var ema20 = e5CalcEma_(closes, 20);
  var ema50 = e5CalcEma_(closes, 50);
  var n = closes.length - 1;
  var price = closes[n];
...
function e5BiasLabelJa_(bias) {
  if (bias === 'bullish') return '上昇';
  if (bias === 'bearish') return '下降';
  return '中立';
}

function e5DonchianHigh_(candles, lookback, endIdx) {
...
function e5DonchianLow_(candles, lookback, endIdx) {
  var start = Math.max(0, endIdx - lookback);
