...
function jCalcTrapRoundProfit_(pair, buyPrice, trapStep, amount, role) {
  role = role || 'maker';
  var tp = jCalcTakeProfitPrice_(pair, buyPrice, amount, trapStep, { feeRoleProfit: role });
 ...
 */
function jGetSlipWidth_(pair, price) {
  var map = J_CONFIG.SLIP_WIDTH_BY_PAIR || {};
 ...
 */
function jCalcTakeProfitPrice_(pair, buyPrice, amount, trapStep, cfg) {
  cfg = cfg || jGetConfig_();
  var role = cfg.feeRoleProfit || J_CONFIG.FEE_ROLE_FOR_PROFIT || 'maker';
  var feeWidth = jMinProfitableTrapStep_(pair, buyPrice, amount, role);
  var slipWidth = jGetSlipWidth_(pair, buyPrice);
  var step = Number(trapStep) || 0;
  var width = step + feeWidth + slipWidth;
  if (width <= 0) width = jGetSlipWidth_(pair, buyPrice) || 0.01;
  return {
    sellPrice: jRoundPrice_(pair, buyPrice + width),
    trapStep: step,
    feeWidth: feeWidth,
    slipWidth: slipWidth,
    width: width,
...
function jCalcWorstCaseGridJpy_(pair, lastPrice, plan, levelAmount, role) {
  ...
  if (plan.levelPrices && plan.levelPrices.length) {
    ...
    for (var k = 0; k < plan.levelPrices.length; k++) {
      sum += jCalcBuyCostJpy_(pair, plan.levelPrices[k], levelAmount, role);
    }
    return sum;
  }
  var total = 0;
