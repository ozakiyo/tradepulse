 ...
 * @param {string} pairMode 'grid' | 'single' | 'symbol' | 'symbolFifo' | 'gcfxFxRange'
 */
function rptCollectTradesFromSheet_(sheetName, colMap, from, to, pairMode) {
  ...
  if (pairMode === 'gcfxFxRange') return rptCollectGcfxFxRangeTradesForPeriod_(sheetName, colMap, from, to);
...
function rptGcfxFxPnlJpy_(symbolLabel, side, entryPrice, exitPrice, units, cfg) {
  var pairId = gcfxPairIdFromSymbol_(symbolLabel);
  if (pairId) {
    return gcfxCalcPnlJpy_(pairId, side, entryPrice, exitPrice, units, cfg);
  }
...
/** G-CFX: 半分損切を含む1エントリーを1取引として PF・勝率を集計 */
...
/** チームG-CFX: 全履歴を FIFO ペアリング（半分損切→残り決済・再エントリー対応） */
