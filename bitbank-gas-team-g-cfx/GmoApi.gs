...
function gmoGetCandles1h_(symbol, minBars) {
  minBars = minBars || GCFX_CONFIG.MIN_CANDLES_1H;
  var runKey = symbol + '_1h_' + minBars;
...
function gmoGetCandles5m_(symbol, minBars) {
  minBars = minBars || GCFX_CONFIG.MIN_CANDLES_5M;
  var runKey = symbol + '_5m_' + minBars;
  if (gmoCandlesRunCache_[runKey]) return gmoCandlesRunCache_[runKey];
  var candles = gmoGetCandles_(symbol, '5min', minBars);
  gmoCandlesRunCache_[runKey] = candles;
  return candles;
}
...
function gmoPlaceMarketOpen_(symbol, gcfxSide, size) {
  symbol = gmoNormalizeLeverageSymbol_(symbol);
  return gmoPrivateRequest_('post', '/v1/order', null, {
    symbol: symbol,
    side: gmoSideFromGfx_(gcfxSide),
    executionType: 'MARKET',
    size: String(size),
  });
}

function gmoPlaceMarketClose_(symbol, gcfxSide, positionId, size) {
  symbol = gmoNormalizeLeverageSymbol_(symbol);
  return gmoPrivateRequest_('post', '/v1/closeOrder', null, {
    symbol: symbol,
    side: gmoCloseSideFromGfx_(gcfxSide),
    executionType: 'MARKET',
    settlePosition: [
      {
...
function gmoFindOpenPosition_(symbol, gcfxSide, sizeHint) {
  Utilities.sleep(400);
  symbol = gmoNormalizeLeverageSymbol_(symbol);
  var want = gmoSideFromGfx_(gcfxSide);
