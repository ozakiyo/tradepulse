 ...
 * @param {string} pairMode 'grid' | 'single' | 'symbol' | 'symbolFifo' | 'gfxFxRange'
 */
function rptCollectTradesFromSheet_(sheetName, colMap, from, to, pairMode) {
  ...
  if (pairMode === 'gfxFxRange') return rptCollectGfxFxRangeTradesForPeriod_(sheetName, colMap, from, to);
...
function rptCollectGfxFxRangeTradesForPeriod_(sheetName, colMap, from, to) {
  var allRows = rptSortTradeRowsByTime_(rptReadAllTradeRowsFromSheet_(sheetName, colMap));
  var paired = rptPairGfxFxRangeTrades_(allRows);
  var result = [];
...
function rptGfxFxPnlJpy_(symbolLabel, side, entryPrice, exitPrice, units, cfg) {
  var pairId = gfxPairIdFromSymbol_(symbolLabel);
  if (pairId) {
    return gfxCalcPnlJpy_(pairId, side, entryPrice, exitPrice, units, cfg);
  }
...
function rptGfxFxRoundTripKey_(trade) {
  return [trade.symbol, trade.side, trade.entryTime, trade.entryPrice].join('\0');
}

/** G-FX: 半分損切を含む1エントリーを1取引として PF・勝率を集計 */
function rptCalcGfxFxStats_(trades) {
  var groups = {};
  var groupHoldH = {};
  var openCount = 0;
  ...
  for (var i = 0; i < trades.length; i++) {
    ...
    var key = rptGfxFxRoundTripKey_(t);
...
function rptGfxFxLotKey_(symbol, side) {
  return symbol + '\0' + side;
}

function rptGfxFxPushLot_(queues, symbol, side, row) {
  var key = rptGfxFxLotKey_(symbol, side);
...
function rptGfxFxConsumeClose_(queues, symbol, side, row, cfg, trades) {
  var key = rptGfxFxLotKey_(symbol, side);
  ...
  while (remainToClose > 0.0001 && queue.length) {
    ...
    var pnlJpy = rptGfxFxPnlJpy_(symbol, side, lot.price, row.price, closedUnits, cfg);
    var pnlPct =
      side === 'long'
...
/** チームG-FX: 全履歴を FIFO ペアリング（半分損切→残り決済・再エントリー対応） */
function rptPairGfxFxRangeTrades_(rows) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (label.indexOf('ロング新規') >= 0) {
      rptGfxFxPushLot_(queues, sym, 'long', r);
    } else if (label.indexOf('ショート新規') >= 0) {
      rptGfxFxPushLot_(queues, sym, 'short', r);
    } else if (label.indexOf('ロング決済') >= 0) {
      rptGfxFxConsumeClose_(queues, sym, 'long', r, cfg, trades);
    } else if (label.indexOf('ショート決済') >= 0) {
      rptGfxFxConsumeClose_(queues, sym, 'short', r, cfg, trades);
    }
  }
