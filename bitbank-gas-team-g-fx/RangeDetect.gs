function gfxRangeWidthPct_(high, low) {
...
function gfxDetectDailyRange_(candlesDaily, cfg) {
  ...
  var widthPct = gfxRangeWidthPct_(high, low);
...
function gfxDetectH1Range_(candles1h, daily, cfg) {
...
function gfxIsNearLower_(price, h1Low, touchPct) {
  var band = h1Low * (touchPct / 100);
  return price <= h1Low + band;
}

function gfxIsNearUpper_(price, h1High, touchPct) {
  var band = h1High * (touchPct / 100);
  return price >= h1High - band;
}

function gfxIsDailyBreak_(close, daily) {
  if (!daily.high || !daily.low) return false;
  return close > daily.high || close < daily.low;
}

function gfxIsDailyAdverseBreak_(close, daily, side) {
  if (!daily.high || !daily.low) return false;
...
function gfxIsH1AdverseBreak_(close, h1, side) {
...
function gfxCalcTakeProfit_(side, h1, tpRatio) {
  if (!h1 || !h1.high || !h1.low || h1.high <= h1.low) return null;
  var w = h1.high - h1.low;
  var ratio = gfxClampTpRatio_(tpRatio);
  if (side === 'long') return h1.low + w * ratio;
  return h1.high - w * ratio;
}

function gfxIsAtOrPastTakeProfit_(price, tpPrice, side, touchPct) {
