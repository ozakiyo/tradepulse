#!/usr/bin/env node
/**
 * 経集 JSON の observe / action / quote を現代語に整える
 * 用法: node scripts/modernize-suttanipata.mjs [snp01|...|all]
 */
...
const DATA = path.join(__dirname, '../public/dhamma/data/suttanipata');
...
const files = arg === 'all'
  ? Array.from({ length: 5 }, (_, i) => `snp${String(i + 1).padStart(2, '0')}.json`)
  : [`${arg}.json`];
