#!/usr/bin/env node
/**
 * 小部 JSON の observe / action / quote を現代語に整える
 * 用法: node scripts/modernize-khuddaka.mjs [k01|...|all]
 */
...
const DATA = path.join(__dirname, '../public/dhamma/data/khuddaka');
...
const files = arg === 'all'
  ? Array.from({ length: 15 }, (_, i) => `k${String(i + 1).padStart(2, '0')}.json`)
  : [`${arg}.json`];
