...
set_kv() {
  ...
  if grep -q "^${key}=" "$ENV_SERVER" 2>/dev/null; then
    sed -i "s|^${key}=.*|${key}=${val}|" "$ENV_SERVER"
  else
...
grep -E '^(SAXO_ACCOUNT_KEY|GSAXO_(DRY_RUN|INCLUDE_INDEX|EXCLUDE_HEAVY_FX|PAPER_JPY|MAX_OPEN_POSITIONS|DAILY_STOP_COOLDOWN_HOURS|TREND_FILTER|TREND_AUTO|META_LEAGUE|ADX_|ER_))=' "$ENV_SERVER"
echo ""
