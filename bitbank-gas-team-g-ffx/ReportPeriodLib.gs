 ...
 * @param {string} pairMode 'grid' | 'single' | 'symbol' | 'symbolFifo' | 'gffxFxRange'
 */
function rptCollectTradesFromSheet_(sheetName, colMap, from, to, pairMode) {
  ...
  if (pairMode === 'gffxFxRange') return rptCollectGcfxFxRangeTradesForPeriod_(sheetName, colMap, from, to);
...
function rptGcfxFxPnlJpy_(symbolLabel, side, entryPrice, exitPrice, units, cfg) {
  var pairId = gffxPairIdFromSymbol_(symbolLabel);
  if (pairId) {
    return gffxCalcPnlJpy_(pairId, side, entryPrice, exitPrice, units, cfg);
  }
...
/** G-FFX: 半分損切を含む1エントリーを1取引として PF・勝率を集計 */
...
/** チームG-FFX: 全履歴を FIFO ペアリング（半分損切→残り決済・再エントリー対応） */
