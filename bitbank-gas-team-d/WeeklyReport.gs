/**
 * チームD: 週次レポート
 * D4_売買履歴から 7日間 / 前日 / 今月 を集計。
 */

var D4_SHEET_REPORT = 'D4_週次レポート';
var D4_SHEET_DAILY = 'D4_日次レポート';
var D4_SHEET_MONTHLY = 'D4_月次レポート';
var D4_REPORT_MAX_TRADES = 100;
var D4_TRADE_COL = { time: 0, side: 1, price: 2, amount: 3, memo: 5 };

function d4CollectReportTrades_() {
  var range = rptGetPeriodRange_('7d');
  return d4CollectReportTradesForPeriod_(range.from, range.to);
}

function d4CollectReportTradesForPeriod_(from, to) {
  return rptCollectTradesFromSheet_(D4_SHEET_TRADE, D4_TRADE_COL, from, to, 'single');
}
...
function d4AnalyzeTrades_(trades) {
  var results = [];
  for (var i = 0; i < Math.min(trades.length, D4_REPORT_MAX_TRADES); i++) {
    var t = trades[i];
...
function d4GenerateRecommendations_(analyses) {
  var recs = [];
  var closed = [];
  var wins = 0;
  var totalHold = 0;
  ...
  if (closed.length > 0 && wins / closed.length < 0.4) {
    recs.push({
      ...
      proposed: '柴田法則フィルター強化',
      reason: '勝率40%未満 — KAGI_BASE_STEP_JPYの調整または法則条件の追加を検討',
    });
  }
...
function d4WritePeriodSheet_(sheetName, title, trades, stats) {
  ...
  rptWriteTradeDetailBlock_(sheet, row, trades, D4_REPORT_MAX_TRADES);
}

function d4WriteReport_(trades, analyses, recs, periodBundle) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName(D4_SHEET_REPORT);
  if (!sheet) sheet = ss.insertSheet(D4_SHEET_REPORT);
  sheet.clear();
  ...
  d4Log_('週次レポート出力完了 (' + trades.length + '件)');
}
...
function d4RunReport_() {
  var range7 = rptGetPeriodRange_('7d');
  var rangeY = rptGetPeriodRange_('yesterday');
  var rangeM = rptGetPeriodRange_('month');

  var trades7d = d4CollectReportTradesForPeriod_(range7.from, range7.to);
  var tradesY = d4CollectReportTradesForPeriod_(rangeY.from, rangeY.to);
  var tradesM = d4CollectReportTradesForPeriod_(rangeM.from, rangeM.to);
  ...
  if (trades7d.length === 0 && tradesY.length === 0 && tradesM.length === 0) {
    ...
    d4ReportToMeta_('7日間', 0, '-', '-', 0, 0, '取引なし');
    d4ReportToMeta_('前日', 0, '-', '-', 0, 0, '取引なし');
    d4ReportToMeta_(rangeM.label, 0, '-', '-', 0, 0, '取引なし');
    return;
  }

  var analyses = d4AnalyzeTrades_(trades7d);
  var recs = d4GenerateRecommendations_(analyses);
  d4WriteReport_(trades7d, analyses, recs, periodBundle);
  d4WritePeriodSheet_(D4_SHEET_DAILY, '【D4 前日レポート】', tradesY, statsY);
  d4WritePeriodSheet_(D4_SHEET_MONTHLY, '【D4 月次レポート】', tradesM, statsM);
...
function d4ReportToMeta_(period, tradeCount, winRate, pf, netPnl, avgHoldH, recommendation) {
  ...
  try {
    ...
    d4Log_('META統合レポートに送信(' + period + ')');
  } catch (e) {
    d4Log_('META統合レポート送信失敗: ' + e.message);
  }
}
