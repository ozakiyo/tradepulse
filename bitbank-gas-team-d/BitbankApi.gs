function d4Sign_(nonce, message) {
...
function d4PrivateRequest_(method, path, query, bodyObj) {
  ...
  var signature = d4Sign_(nonce, signMsg);
  var url = D4_CONFIG.PRIVATE_API + path + (query ? '?' + query : '');
  var options = {
    method: method,
...
function d4PublicGet_(path) {
  var url = D4_CONFIG.PUBLIC_API + path;
  var res;
  try {
...
function d4GetFetchDayCount_(candleType, minBars) {
  var barsPerDay = candleType === '15min' ? 96 : 24;
  var need = Math.ceil(minBars / barsPerDay) + 1;
  var cap =
    candleType === '15min'
      ? D4_CONFIG.CANDLE_FETCH_MAX_DAYS_15M || 3
      : D4_CONFIG.CANDLE_FETCH_MAX_DAYS || 5;
  return Math.min(cap, Math.max(need, 1));
}

function d4DedupeCandles_(all) {
...
function d4GetTicker_() {
  var cache = CacheService.getScriptCache();
  var ckey = 'd4_ticker_' + D4_CONFIG.PAIR;
  ...
  var data = d4PublicGet_('/' + D4_CONFIG.PAIR + '/ticker');
  var ticker = { last: Number(data.last), high: Number(data.high), low: Number(data.low) };
  cache.put(ckey, JSON.stringify(ticker), D4_CONFIG.TICKER_CACHE_TTL_SEC || 60);
  return ticker;
}

function d4FetchCandlesDay_(daysAgo, candleType) {
  var d = new Date();
  d.setDate(d.getDate() - daysAgo);
  var ymd = Utilities.formatDate(d, 'Asia/Tokyo', 'yyyyMMdd');
  var type = candleType === '1hour' ? '1hour' : '15min';
  var cache = CacheService.getScriptCache();
  var ckey = 'd4_cd_' + D4_CONFIG.PAIR + '_' + type + '_' + ymd;
  var cached = cache.get(ckey);
  if (cached) {
    try {
      return JSON.parse(cached);
  ...
  var data = d4PublicGet_('/' + D4_CONFIG.PAIR + '/candlestick/' + type + '/' + ymd);
...
function d4GetCandles_(cfg) {
  var candleType = (cfg && cfg.candleType) || '15min';
  var minBars = (cfg && cfg.minCandles) || d4GetMinCandles_(candleType);
  var cache = CacheService.getScriptCache();
  var bkey = 'd4_cb_' + D4_CONFIG.PAIR + '_' + candleType + '_' + minBars;
  ...
  var maxDays = d4GetFetchDayCount_(candleType, minBars);
  var all = [];
  for (var daysAgo = 0; daysAgo < maxDays; daysAgo++) {
    var day = d4FetchCandlesDay_(daysAgo, candleType);
    if (day.length) all = day.concat(all);
    if (all.length >= minBars) break;
  }
  var deduped = d4DedupeCandles_(all);
  if (deduped.length >= minBars) {
    cache.put(bkey, JSON.stringify(deduped), D4_CONFIG.CANDLE_CACHE_TTL_SEC || 600);
  }
  return deduped;
}

/** @deprecated d4GetCandles_(cfg) を使用 */
function d4GetCandles1h_() {
  return d4GetCandles_(d4GetConfig_());
}

function d4InitPaperWallet_(state) {
  var cap = Number(
    PropertiesService.getScriptProperties().getProperty('PAPER_JPY') || D4_CONFIG.PAPER_JPY_DEFAULT
  );
...
function d4ApplyPaperTrade_(state, side, price, amount) {
  var w = d4InitPaperWallet_(state);
  var fee = D4_CONFIG.PAPER_FEE_RATE;
  if (side === 'buy' || side === '買い') {
...
function d4GetAssetsForRun_(cfg, state) {
  if (cfg.dryRun) {
    var w = d4InitPaperWallet_(state);
...
function d4GetAssets_() {
  var data = d4PrivateRequest_('get', '/user/assets', null, null);
  var jpy = 0;
  var btc = 0;
...
function d4CancelAllOrders_(cfg) {
  if (cfg.dryRun) {
    d4Log_('[DRY_RUN] 全キャンセル（スキップ）');
    return;
  }
  var data = d4PrivateRequest_('get', '/user/spot/active_orders', 'pair=' + D4_CONFIG.PAIR, null);
  (data.orders || []).forEach(function (o) {
    if (o.user_cancelable) {
      d4PrivateRequest_('post', '/user/spot/cancel_order', null, {
        pair: D4_CONFIG.PAIR,
...
function d4PlaceLimit_(side, price, amount, cfg) {
  var body = {
    pair: D4_CONFIG.PAIR,
    side: side,
    type: 'limit',
    price: String(Math.round(price)),
    amount: amount.toFixed(D4_CONFIG.BTC_AMOUNT_DECIMALS),
  };
  if (cfg.dryRun) {
    d4Log_('[DRY_RUN] ' + side + ' limit price=' + body.price + ' amount=' + body.amount);
    return;
  }
  d4PrivateRequest_('post', '/user/spot/order', null, body);
}
