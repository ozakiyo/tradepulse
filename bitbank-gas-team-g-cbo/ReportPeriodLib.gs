 ...
 * @param {string} pairMode 'grid' | 'single' | 'symbol' | 'symbolFifo' | 'gcboFxRange'
 */
function rptCollectTradesFromSheet_(sheetName, colMap, from, to, pairMode) {
  ...
  if (pairMode === 'symbolFifo') return rptPairSymbolFifoTrades_(rows);
  if (pairMode === 'gcboFxRange') return rptCollectGcfxFxRangeTradesForPeriod_(sheetName, colMap, from, to);
  return rptPairSingleTrades_(rows);
}

function rptReadAllTradeRowsFromSheet_(sheetName, colMap) {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(sheetName);
  if (!sheet || sheet.getLastRow() < 2) return [];
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    rows.push({
      ...
      memo: colMap.memo != null ? String(row[colMap.memo] || '') : '',
      symbol: colMap.symbol != null ? String(row[colMap.symbol] || '').trim() : '',
    });
  }
...
function rptSortTradeRowsByTime_(rows) {
...
function rptCollectGcfxFxRangeTradesForPeriod_(sheetName, colMap, from, to) {
  var allRows = rptSortTradeRowsByTime_(rptReadAllTradeRowsFromSheet_(sheetName, colMap));
  var paired = rptPairGcfxFxRangeTrades_(allRows);
  var result = [];

  for (var i = 0; i < paired.length; i++) {
    var t = paired[i];
    if (t.pnlJpy != null && t.exitTime) {
      if (rptRowInRange_(new Date(t.exitTime), from, to)) result.push(t);
    } else if (t.entryTime && new Date(t.entryTime) < to) {
      result.push(t);
    }
  }
...
/** 銘柄別FIFO（チームG: 買いのみレンジ） */
function rptPairSymbolFifoTrades_(rows) {
  var openBySym = {};
  var trades = [];
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (r.side === '買い') {
      if (!openBySym[sym]) openBySym[sym] = [];
      openBySym[sym].push(r);
      continue;
    }

    if (r.side !== '売り' || !openBySym[sym] || !openBySym[sym].length) continue;

    var entry = openBySym[sym].shift();
    var pnlJpy = (r.price - entry.price) * entry.amount;
    trades.push({
      symbol: sym,
      entryTime: entry.time,
  ...
  Object.keys(openBySym).forEach(function (sym) {
    var queue = openBySym[sym];
...
function rptGcfxFxPnlJpy_(symbolLabel, side, entryPrice, exitPrice, units, cfg) {
  var pairId = gcboPairIdFromSymbol_(symbolLabel);
  if (pairId) {
    return gcboCalcPnlJpy_(pairId, side, entryPrice, exitPrice, units, cfg);
  }
  var diff = side === 'long' ? exitPrice - entryPrice : entryPrice - exitPrice;
  return diff * units;
}

function rptGcfxFxRoundTripKey_(trade) {
  return [trade.symbol, trade.side, trade.entryTime, trade.entryPrice].join('\0');
}

/** G-CBO: 半分損切を含む1エントリーを1取引として PF・勝率を集計 */
function rptCalcGcfxFxStats_(trades) {
  var groups = {};
  var groupHoldH = {};
  var openCount = 0;
  ...
  for (var i = 0; i < trades.length; i++) {
    ...
    var key = rptGcfxFxRoundTripKey_(t);
    groups[key] = (groups[key] || 0) + t.pnlJpy;
    if (t.entryTime && t.exitTime) {
      var holdH = (new Date(t.exitTime).getTime() - new Date(t.entryTime).getTime()) / 3600000;
  ...
  for (var k = 0; k < keys.length; k++) {
    ...
    if (groupHoldH[keys[k]]) totalHold += groupHoldH[keys[k]];
  }
...
function rptGcfxFxLotKey_(symbol, side) {
  return symbol + '\0' + side;
}

function rptGcfxFxPushLot_(queues, symbol, side, row) {
  var key = rptGcfxFxLotKey_(symbol, side);
...
/** 決済行を FIFO ロットに割当（半分損切・残り決済に対応） */
function rptGcfxFxConsumeClose_(queues, symbol, side, row, cfg, trades) {
  var key = rptGcfxFxLotKey_(symbol, side);
  var queue = queues[key];
  if (!queue || !queue.length) return;

  var remainToClose = row.amount > 0 ? row.amount : queue[0].amount;

  while (remainToClose > 0.0001 && queue.length) {
    var lot = queue[0];
    var closedUnits = Math.min(remainToClose, lot.amount);
    var pnlJpy = rptGcfxFxPnlJpy_(symbol, side, lot.price, row.price, closedUnits, cfg);
    var pnlPct =
      side === 'long'
...
/** チームG-CBO: 全履歴を FIFO ペアリング（半分損切→残り決済・再エントリー対応） */
function rptPairGcfxFxRangeTrades_(rows) {
  ...
  for (var i = 0; i < rows.length; i++) {
    var r = rows[i];
    var sym = r.symbol || '_';
    var label = String(r.side || '');

    if (label.indexOf('ロング新規') >= 0) {
      rptGcfxFxPushLot_(queues, sym, 'long', r);
    } else if (label.indexOf('ショート新規') >= 0) {
      rptGcfxFxPushLot_(queues, sym, 'short', r);
    } else if (label.indexOf('ロング決済') >= 0) {
      rptGcfxFxConsumeClose_(queues, sym, 'long', r, cfg, trades);
    } else if (label.indexOf('ショート決済') >= 0) {
      rptGcfxFxConsumeClose_(queues, sym, 'short', r, cfg, trades);
    }
  }
  ...
  Object.keys(queues).forEach(function (key) {
    ...
    queues[key].forEach(function (lot) {
      ...
      trades.push({
        ...
        pnlJpy: null,
        pnlPct: null,
        reason: side === 'long' ? '保有中(ロング)' : '保有中(ショート)',
      });
    });
  });
