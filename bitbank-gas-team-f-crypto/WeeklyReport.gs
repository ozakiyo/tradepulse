...
function f6RunReport_() {
  ...
  if (trades7d.length === 0 && tradesY.length === 0 && tradesM.length === 0) {
    ...
    f6ReportToMeta_('F-Crypto', '7日間', 0, '-', '-', 0, 0, '取引なし');
    f6ReportToMeta_('F-Crypto', '前日', 0, '-', '-', 0, 0, '取引なし');
    f6ReportToMeta_('F-Crypto', rangeM.label, 0, '-', '-', 0, 0, '取引なし');
    return;
  }
  ...
  f6WritePeriodSheet_(F6_SHEET_DAILY, '【F-Crypto 前日レポート】', tradesY, statsY);
  f6WritePeriodSheet_(F6_SHEET_MONTHLY, '【F-Crypto 月次レポート】', tradesM, statsM);
